...
export const CodeHighlight = memo(
  ({ className, children, node, ...props }: CodeHighlightProps) => {
    ...
    // In the real app, this would come from a theme context
    ...
    // Use cached version if available, otherwise use the current highlighted code
    ...
    return (
      <div className="relative group">
        <div className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={handleCopy}
            className="p-2 rounded-md bg-background/80 hover:bg-background border shadow-sm"
            title="Copy code"
          >
            {copied ? (
        ...
        <pre
          className={`relative overflow-x-auto rounded-lg border bg-muted p-4 ${
            className || ""
          }`}
          {...props}
        >
